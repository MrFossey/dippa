...
\tikzset{
  ...
  syst/.style={base, cylinder, text width=5em}, % aspect = 2
  file/.style={proc, rounded corners},
...
\node [syst] (s0) {Production};
\node [proc, join] (p0) {Test case generation};
\node [file, join] (f1) {Integration test case};
\node [syst, join] (e0) {Test case execution engine};
\node [syst, join] (s1) {SUT};
\node [syst, join] (e1) {Surrogate engine};
\node [file, right=of s0, join] (f0) {Model};
\node [proc, right=of f1, join] (p1) {Verification};
\node [file, join] (f2) {Execution trace};
\node [proc] (p2) {Trace collecting};
\begin{comment}
...
\node [syst, join] (t1) {Got msg?};
% No join for exits from test nodes - connections have more complex
% requirements
% We continue until all the blocks are positioned
\node [proc] (p2) {$k \mathbin{{-}{=}} 1$};
\node [proc, join] (p3) {Dispatch message};
\node [syst, join] (t2) {Got msg?};
\node [syst] (t3) {Capacity?};
\node [syst] (t4) {$k \mathbin{{-}{=}} 1$};
...
\path (s0.south) to node [near start, xshift=1em] (p0);
  \draw [*->,lcnorm] (s0.south) -- (p0);
\path (p0.south) to node [near start, xshift=1em] (f1);
  \draw [*->,lcnorm] (p0.south) -- (f1); 
\path (f1.south) to node [near start, xshift=1em] (e0);
  \draw [*->,lcnorm] (f1.south) -- (e0); 
\path (e0.south) to node [near start, xshift=1em] (s1);
  \draw [*->,lcnorm] (e0.south) -- (s1); 
\path (s1.south) to node [near start, xshift=1em] (e1);
  \draw [*->,lcnorm] (s1.south) -- (e1);

\path (f0.south) to node [near start, xshift=1em] (p0);
  \draw [*->,lcnorm] (f0.south) -- (p0);
\path (f1.east) to node [near start, xshift=1em] (p1);
  \draw [*->,lcnorm] (f1.east) -- (p1);
\path (s1.east) to node [near start, xshift=1em] (p2);
  \draw [*->,lcnorm] (s1.east) -- (p2); 
\path (p1.south) to node [near start, xshift=1em] (e0);
  \draw [*->,lcnorm] (p1.south) -- (e0);

\path (p2.north) to node [near start, xshift=1em] (f2);
  \draw [*->,lcnorm] (p2.north) -- (f2); 
\path (f2.north) to node [near start, xshift=1em] (p1);
  \draw [*->,lcnorm] (f2.north) -- (p1);
  
%\path (s0.east) to node [near start, yshift=1em] {$n$} (c1); 
%  \draw [o->,lcfree] (s0.east) -- (c1);

%\path (t1.south) to node [near start, xshift=1em] {$y$} (p2);
%  \draw [*->,lcnorm] (t1.south) -- (p2);
%\path (p0.south) to node [near start, xshift=1em] {$y$} (p1);
%  \draw [*->,lcnorm] (p0.south) -- (p1);
...
% =================================================
INSERT FIGURE HERE
\begin{comment} % <---- REMOVE THIS LINE
\pgfdeclarelayer{marx}
\pgfsetlayers{main,marx}
% A macro for marking coordinates (specific to the coordinate naming
% scheme used here). Swap the following 2 definitions to deactivate
...
% positioning of nodes
\tikzset{
  system/.style={draw, on chain, on grid, align=center, minimum height=4ex},
  process/.style={base, rectangle, text width=8em},
  file/.style={process, cylinder, text width=5em},
  engine/.style={file},
  % coord node style is used for placing corners of connecting lines
% coord/.style={coordinate, on chain, on grid, node distance=6mm and 25mm},
  % nmark node style is used for coordinate debugging marks
...
% -------------------------------------------------
% Start by placing the nodes
\node [system] (s0) {Production};
% Use join to connect a node to the previous one 
\node [file, join] (f0) {Model};
\node [process, join] (p0) {Test case generation};
%\node [file] (f1) {Integration test case};
%\node [process] (p1) {Verification};
%\node [engine] (e0) {Test case execution engine};
% No join for exits from test nodes - connections have more complex
% requirements
% We continue until all the blocks are positioned
%\node [system] (s1) {SUT};
%\node [engine] (e1) {Surrogate engine};
%\node [file] (f2) {Execution trace};
%\node [process] (p2) {Trace transformation};
%\node [process] (p3) {Trace collecting};
% -------------------------------------------------
% A couple of boxes have annotations
% \node [above=0mm of p4, it] {(Queue was empty)};
% -------------------------------------------------
...
% draw a path with a (consistently positioned) annotation node, then
% we draw the arrow itself.
\path (s0.south) to node [near start, xshift=1em] {$y$} (p0);
  \draw [*->,lcnorm] (s0.south) -- (p0);
\end{comment} % <---- REMOVE THIS LINE
\begin{comment}
% ------------------------------------------------- 
% Now the straight east-west connections. To provide consistent
% positioning of the test exit annotations, we have positioned
