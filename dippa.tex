...
Huang et al's model, a precursor to the authors unified testing framework presented above, is more concerned with the testing environment arrangements, but \ref{fig:huang} already shows a clear framework structure spanning from test case generation to verification of functionality \citep{huang2008surrogate}. 
...
\ref{fig:funnel} presents a testing framework wireframe. Testing objectives and the general approach are defined in a testing plan or strategy. Typically, a plan includes ... (reference). Requirements are generally set by specifications of functionality or agreements on e.g. policy or service level. The teting process also requires an executable set of test cases, which should be constructed with regard to the stratey scope. These can be derived (even autmatically) from formally structured models or specifications. 
