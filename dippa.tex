...
\citet{pezze2008software} point out that big-bang essentially merges integration testing with system testing. While it reduces costs of 'test scaffolding', the structure (e.g. stubs and drivers) needed to carry out piecemeal testing
...
\tikzset{
  base/.style={draw, on chain, on grid, align=center, minimum height=4ex},
  label/.style={draw, on chain, on grid, align=center, minimum height=4ex},
  proc/.style={base, rectangle, minimum height=2em, text width=7em},
  box/.style={base, circle, text width=5em, fill = blackish, text = white},
  line/.style={draw, thick, -latex'}
}
% -------------------------------------------------
% Placing the nodes
\node [proc] (p0) {Test case generation};
\node [proc] (p1) {Surrogate engine};
\node [proc] (p2) {Running tests};
\node [proc] (p3) {Trace collecting};
\node [proc] (p4) {Verification};

\node [proc, right=of p2] (p5) {Scheduling};
\node [proc, right=of p3] (p6) {Trace transform};
...
The framework includes a visualization layer and GUI, an execution engine which also collects execution traces. The engine, capable of running tests, is an IDE or integrated development environment which is also used in constructing the test scripts, both manually and through capture and playback. 

The IDE environment is also capable of producing surrogates, which simulate the behaviour of actual systems. These mock service surrogates allow verification by providing an otherwise absent execution trail or returning courtesy messages of the test outcome. Building and running mock services can also be done with capture and replay, which contributed to the tool choice.

Execuction engine duties are split between the actual running of the tests and scheduling test cycles. 

The experiment quickly showed the difficulty of dividing responsibility between software components. Implementing one feature requires cooperation between tools where one assumes part of the duties while others fulfil other parts. In true irony, implementing the integration testing framework already created a number of integrations to be tested.
...
Specification of sufficient accuracy for deriving test cases were not available, and therefore test scripts are based on capture. Test cases are written in the soapui format but since their structural composition is largely that of an XML file, their reuasability and low risk of vendor lock-in is justified. Execution traces are simply full text accounts of test results. 

An initial oversight, visualizing the test results is added as a process to the overall framework process.
...
The framework satiesfies the first criteria esaily. Tests can be grouped and executing packages of tests in Jenkins is done with a simple click. 

The second criteria is also arguable satisfied since integration testing. Is satisfied by inputting information requried by the test first and lastly deleting the information. Alternatively, unchanged information may be used, though this is a more risky approach. 
