...
\subsection{Test case creation and model-based testing}
...
\citet{linnenkugel1990test} present a flow-oriented coverage hierarchy of node, relation, call, sequence, and loop coverage measures. For integration testing, individual components (modules) are nodes and integrations are relations(call statements). Coverage criteria include:
\begin{itemize}
...
\end{itemize}
Here, complex path sequences can be interpreted as system tests, whereas \emph{all-relations} and \all{multiple-relations} fall under integration testing.

% selecting which part of the system to test
Testing must provide reasonable confidence in system performance and as such can be selective where only a part of the whole system is examined in order save time and costs. It is essential to model and understand the system flow and which part of it should be tested. 

An alternative way to target testing, a code-based approach look for modified and related code which is then tested, typically in unit tests. Testing can also be targeted by risk analysis. Risk areas are ones most affected by changes and which require the best coverage and more tests. In state-based regression testing coverage criteria are determined and test cases devised using control and data flow descriptions in UML state diagrams. Lastly, models, such as UML diagrams, can be used implemented regression testing for instance when access to code is not available. \citep{bhuyan2012survey}
