...
\chapter{Testing system integrations}
...
First off, production records are used for the genration of new test cases. This means that there must be some way to capture and record behaviour in the production environment, and save the information in a database. Models, diagrams, and specifications 

In the following generation step, the records are turned into an executable test case, which are then run by a testing engine. Different types of services must be turned into cases in a way that the engine can run them. 
...
\end{tikzpicture}
\caption{Testing framework, adapted from \citep{liu2009unified}} \label{fig:UTF}
\end{figure}
% =================================================
