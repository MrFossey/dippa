...
There are a total of six processes and three objects involved in the the proposed framework, along with the system under test (SUT) and test resource database. The processes are divided into two steps, a preparatory one, and an execution step. In the preparation step a new test resource, that is to say a test case \emph{or} a mock service, is created first by capturing message records, and secondly using these to generate the desired test resource. Befire being added to the test resource database, created cases and surrogates are put through the execution step to make sure they work without error.

The framework enables automated integration testing by scheduling test runs. Each run utilizes 
