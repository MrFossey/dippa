...
Tests that are explicitly deemed important. E.g. some area that critical, has been recently changed, is prone to bugs or is known to have caused failures.
A set of breadth tests that roughly cover all features in test scope. This will give minimum satisfaction of test objectives.
Test for most important functions. Function importance may be attributed to e.g. message traffic, financial impact, etc.
Tests that are easy to automate. Good for quick wins and return for investment. Also quickly ramps up the framework construction.
Test that will give quickest payback. Implement test cases in projects which would otherwise require a lot of manual testing.
Test that are run most often. If tests need to be run often, the tested area is likely subject to many changes thereby increasing risk.
