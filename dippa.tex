...
Software development is an assembly task where components have to be laid carefully for seamless interoperation. This thesis examines testing these cross-component and cross-system connections within a Finnish power solutions supplier and sets out to define a practical\emph{modus operandi} for conducting automated integration testing.

The thesis expounds on integration testing theory and models. Existing testing solutions were deconstructed and carefully analyzed and their best features collected. These practical insights were combined with theoretical findings and crystallized into an integration testing framework, which was also tailored for case company needs. The resulting framework was then comprehensviely evaluated through requirements analysis, heuristic appraisal, and qualitative validation by company experts. 

The framework construct successfully both describes testing process work flow and suggests technical implementation. It extends the popular notion that integration testing is a mere transitional period in development and examines testing as a continuous process all the way from a strategic level to the mundane. The result is a cohesive narrative and guide to implementing automated integration testing.

% Framework OK. Capture and replay.

% integration testing in organizational context
% a framework combining process workflow and a suggested technical implementation validated through proof of concept
% broader view of integration testing, advocated beyond transitiional period in development, through continious intgegration
% presents a construct
% holistically, from strategy to component level, verstaile validation
...
Ohjelmistot rakentuvat useiden komponenttien ja järjestelmien yhteistoiminnan kautta. Näiden liittymien ja rajapintojen toimivuus ja toimintavarmuus on tärkeää. Tämä diplomityö tutkii, mitä automatisoidulta integraatiotestaukselta edellytetään ja, perustuen kohdeyrityksen vaatimuksiin, minkälainen toimintatapa pystyy vastaamaan integraatiotestaustarpeeseen.

Tutkimus pureutui ohjelmistotestausmalleihin ja -teorioihin. Se tarkasteli olemassa olevia testikehyksiä ja perusteellisesti selvitti mistä osista kehykset koostuvat. Näistä käytännön ja testausteorian havainnosta rakennettiin uusi testikehys, joka myös perustui kohdeyrityksen vaatimuksiin. Kehyshypoteesia arvioitiin huolellisesti testauksen vaatimusten, ohjelmistokehyksien heuristiikkojen ja asiantuntija lausuntojen avulla.

Työn tuloksena on suositus testikehyksen työnkululle ja prosesseille sekä kehyksen käytännön toteutukselle. Kehyksen tausta-ajatuksena on myös haastaa yleinen käsityksen siitä, että integraatiotestaus on pelkkä ohjelmistokehityksen välivaihe. Se tarkasteleekin testausta jatkuvana prosessina ja aina strategiselta tasolta päivittäiseen ja komponenttitasoon. Tutkimuksen kaari muodostaa ehyen kertomuksen ja käsikirjan automatisoiden integraationtestauksen toteuttamiseen.
...
% mega todo: the testing process now, evaluation through comparison to requirements, framework comparison excel, pitfalls integration, fill the empty part 4.3, 
...
Succinctly speaking the testing framework requirements include a test case / surrogate generator, a test engine, and an oracle verifying test results. The generation is based on capture replay or formal models which allow for computational execution and thereby automation. WHile test execution is automatic, the creation will likely remain a mostly manual task. 

\begin{adjustwidth}{2.5em}{0pt}
\textbf{2. How should the testing framework be constructed and maintained?} \\
\end{adjustwidth}

The second research question is answered by the framework construct. 
