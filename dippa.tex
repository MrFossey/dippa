...
\begin{tikzpicture} [
   start chain=going below,    % General flow is top-to-bottom
    node distance=10mm and 30mm, % Global setup of box spacing
    ]
% ------------------------------------------------- 
\tikzset{
  base/.style={draw, on chain, on grid, align=center, minimum height=4ex},
  rect/.style={base, rectangle, minimum height=2em, text width=5em},
  line/.style={draw, thick, -latex'}
}

   % \draw[semithick] (0,0) ellipse (4 and 0.5); % funnel top
    \node [rect, xshift=-1.25cm, yshift=1.5cm] (stg) {Strategy};
    \node [rect, xshift=1.25cm, yshift=0.25cm] (req) {Requirements};
    \node [rect] (prc) {Process};
    \node [rect, yshift=-2.0cm] (out) {Outcomes};
    \node [rect, right=of stg, fill= white] (fmw) {Framework};
     
    \draw[semithick] (-4,0) arc (180:0:4 and 0.5);                  % top ellipse top arc

    \draw[dashed,color=gray] (-1,-7) arc (180:0:1 and 0.25);        % bottom ellipse top arc
    \draw[semithick] (-1,-7) arc (-180:0:1 and 0.25);               % bottom ellipse bottom arc
    
    
   
    
    \draw[semithick] (-4,0) arc (-180:0:4 and 0.5);                 % top ellipse bottom arc
    \draw[semithick] (4,0) -- (1,-7);                               % funnel right side
    \draw[semithick] (-4,0) -- (-1,-7);                             % funnel left side
