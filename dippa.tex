...
Oriented Architectures, which is also a key part of Wärtsilä's information architecture, 
esepcially blurs the line between development and integration (HUANG ET AL, weiczorek 
stefanescu roth). Myers introduces six integration strategies: bottom-up, top-down,
...
strategies (Moll & Ammer) where the hierarchy of integrations is built selectively. 

Carey and Bendick discuss build testing [4]. Beizer introduces a
"mixed bag" strategy which combines bottom-up, top-down, big-bang
and build testing [2]. (Lueng & White)

% introduction of stubs and drivers
Delaying testing to the Big Bang is not ideal for a large and complex integration undertaking because examining the integration as a whole and pinpointing discovered defects is difficult. A better option is to test little by little and employ one of the other schemes. Integration-by-integration testing requires drivers and stubs as stand-ins to make up for the absence of actual systems needed for tests.

\section{Automated testing}

Automating testing has obvious benefits, but is not without fault. Implementing automated tests takes a significant investment of time and effort, cannot match the intelligence and intuition of an experienced tester, while some systems undergo so many changes that automation makes little sense. Automation also does not counter inherently flawed software development processes.

primer: automation has its faults

Test case creation is perhaps the most time-consuming task in automated testing. Cases have to be produced based on the test plan and execution strategy, and updated when necessary. Tsai et al. (2001) presented an XML-based framework for automtaed test case generation from WSDL service descriptions.
...
Testing have been proposed, such as mutation testing (delamaro). In mutation 
testing test set quality is evaluated by creating variations of the tested programs and
unning cases against them. If the cases succeed in the mutated program Q, it is considered 
live. Ideally, cases will fail when run against modified, erronous code.

Bai et al., M Di Penta in chaturvedi automated test case generation out of WSDL
...
- A risk area is identified which is considered as the most coverage area that get affected 

by the changes made in the system.

State based regression testing
- A set of coverage criteria is proposed based on control and data flow in UML state diagram 

and it is shown how to generate test cases satisfying these criteria from UML state diagrams.
