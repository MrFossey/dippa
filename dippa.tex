...
First off, messages captured in the production environment are used for the generation of new test cases. This means that there must be some way to record behaviour in production, and save the information in a database or file system. In the following generation step, the records are turned into an executable test case, which are then run by a testing engine against the system under test. 
