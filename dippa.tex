...
Laukkanen, a system to control test assets. Including a database for test data management. Test execution (operating system), monitoring, test design

\citet{fewster1999software} present key characteristics of an automated regime test regime, a system of government of testing which can be likened to a framework but with more emphais on governance and rules of practice. According to the characterstics, in the test regime it should be
\begin{itemize}
...
Automated testing alreades requires the second criteria be satisfied. Assuming the test server and SUT servers are avaialbe, test cases contain necessary information to access information services to be tested. Because some features require inputting information, like orders, which can not be allowed in production, mock services are used. Because the messages travel through the same integration black box the test and the only part that is not "real" is the end point system, outisde of test scope, the tests provide an accurate result of integration functionality and health. A test engineer should monitor execution and react appropriate in case the test cases fail.

Running a test manually and adding the test to the automated pack both require writing or generating the case in the first place. With a good template and guidlines in place the case writing task can be expedited and the engineer adds the case to the correct suite where it will automatically be picked up by the test scripts run through the framework. Everybody wins.
