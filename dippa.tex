...
Requirements and specifications are collected among solution key users and managers. Unit tests are performed by the vendor, which is followed by integration tests in Wärtsilä's test environment executed by Wärtsilä's technical experts possibly aided by the vendor. Next, system tests are done by the solution manager, who then passes the solution forward to business key useres for verification testing. % has verification testing been introduced  

Test cases are constructed manually and are not maintained. After a development project is closed and moves to \eph{live maintanence mode}, new cases are created wwhen needed as new features or enhancements are released. In some few cases, a battery of regression tests is maintained. Testing results and found incidents are logged to a bug tracking system, or collected in shared files or email conversations. 
...
This section examines the components of the framework from an automation perspective. First, test case design and generation, then test execution, and finally verification tools are presented.

\subsection{Test case design and creation}
...
\citet{linnenkugel1990test} present a flow-oriented coverage hierarchy of node, relation, call, sequence, and loop coverage measures. Individual compoenents are nodes and integrations are relations. Complex sequences can be interpreted as system tests. 

all-paths (or path t,esting), requiring each path to be
executed at least once.
all-edges (or branch testing), requiring that each edge
in the flowgraph be visited at least once.
all-nodes (or statement testing), requiring that each
node of the flowgraph be visited at least once.

all-modules requires that every module is executed at
least once, i.e. that every node of the p-graph is covered
(all-nodes).
all-relations requires that the calling relation between
modules is executed at least once, i.e. that at least one
of the edges between two modules is covered (all-oneof-
multiple-edges)
all-multiple-relations requires that every call between
modules is executed at least once, i.e. that every edge
of the p-graph is covered (all-edges).
all-simple-call-sequences requires that every sequence of
(descending) calls without repetition of calls is executed
at least once, i.e. that every simple path of the p-graph
is covered (all-simple-paths).
all-loop-iteratzon-free-call-sequencerse quires that every
sequence of (descending) calls without repetition of loops
is executed at least once, i.e. that every loop-iterationfree
path of the p-graph is covered (all-loop-iterationfree-
paths).
712
all-call-sequences requires that every sequence of (descending)
calls is executed at least once, i.e. that every
path of the p-graph is covered (all-paths).


\citet{benz2007combining} lists different schemes for testing strategy and case coverage based on e.g. traversal in the system network:
\begin{itemize}
\item Manual selection - The tester selects 
...
% test effectiveness and testability
...
weigh benefits against cost, ease of implementation, mechanical tasks automated, creative ones not
pezze: test cases straight from specs, if modeled as finite state machine -> mdoel-based testing pezze p. 152-154
