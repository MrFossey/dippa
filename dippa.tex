...
This section examines the framework-object relationships from an automation perspective. First, test case design and generation, then test execution, and finally verification tools are presented.

\subsection{Test case design and generation}
...
\subsection{Automated testing process}

The testing process is performed by an engine that runs the test cases. Test runs can be scheduled to run at regular intervals or triggered by developer actions, such as a change in code \citep{pezze2008software}. Continuous integration is a development philosophy where integration is reduced to a "nonevent" by integrating new code with old as soon as the former is ready. A big-bang is out of the question. All tests must pass for the build to be accepted. \citep{duvall200continuous} While extreme, the 
...
\section{Piecing together a framework}
...
\chapter{New integration testing framework}
