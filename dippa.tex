...
% ------------------------------------------------------------------
\newcommand{\TITLE}{Developing a framework for continuous testing in software integration in a lifecycle power company}
...
as with conscious minds, there is great chance of misapprehension when two cognitions meet
integration testing in regression testing, disctinction: already together
...
1. How should Wärtsilä's integration testing framework be structured?       % technical perspective
2. What are the requirements for Wärtsilä's integration testing framework   % process perspective
...
\\
\begin{comment}
% 2. Compare alternatives, choose best one. OMIT!
...
This master's thesis study is divided to x parts. First, necessary background information on software integration testing, Wärtsilä's systems architecture, and information systems is presented in Chapter \ref{chapter:background}. Next, integration testing frameworks are explored by looking at existing models and a synthesis on their strategical, tactical, and operational structure is given. Along with a more generic look at software testing theory, Chapter \ref{chapter:framework} breaks down the framework into process components which are then put together in a new framework model tailored for Wärtsilä in Chapter \ref{chapter:frameworkproposal}.
...
The distinction between unit, integration, and system tests is not clear. Some authors consider end-to-end integration testing akin to system tests. Duvall writes about component testing synonymous with integration testing. Pezze, Benz consider this an intermediary stage between unit and integration/system tests while burnsetin likens component testing to unit testing.
...
The \citet{liu2009unified} model (figure \ref{fig:liu}) includes test case generation, case execution, and execution trace collection. The model was used in relatively small-scale experiments, but successfully uncovered bugs early in development. The framework also helped find different kind of bugs and didn't appear to have a bias towards uncovering a certain kind of defect \citep{liu2009unified}.
...
This chapter presents a construct built to address Wärtsilä's testing needs presented in Chapter \ref{chapter:background} and based on theoretical observations made earlier in Chapter \ref{chapter:frameworktheory}. The purpose of the construct is to act as proof of concept and, in true testing form, give early indicators of how valid the theoretical foundations are as well as highlight practical concerns that arise during the experiment.

\section{Testing framework concept structure}

Early on in tool selection it became clear that one tool could not handle all processes but conversly not all processes required their own tool.data Testing software could possible cater to different, even disparate aspects of the framework. The evaluation of testing tools is omitted from this study. The reader is instead referred to excellent resources such as Opensourcetesting.org\footnote{http://www.opensourcetesting.org} and Thoughtworks's CI tool comparison matrix\footnote{http://confluence.public.thoughtworks.org/display/CC/CI+Feature+Matrix}.

The testing processes described in Chapter \ref{chapter:frameworkproposal} are mapped to tools in Figure \ref{fig:structure}. Two tools were seleted: SoapUI for testing and Jenkins for continuous integration, although these further utilize technologies like Apache's Ant and XML.
...
In the framework architecture, SoapUI contains script logic and acts as model while Jenkins provides a user interface for control and view. The experiment quickly showed the difficulty of dividing responsibility between software components. Implementing one feature requires cooperation between tools where one assumes part of the duties while others fullfil other parts. In true irony, implementing the integration testing framework already created a number of integrations to be tested.

Execution engine duties are split between the actual running of the tests and scheduling test cycles. The execution engine runing the tests, SoapUI, also collects execution traces. These are transformed into legible form (simple text) and can be examined in Jenkins. (Accordingly, what can only be called an initial oversight, visualizing the test results is added as a process to the overall framework process.)

SoapUI is an IDE, or integrated development environment, and is also used in constructing test scripts, both manually and through capture and playback. Capture can also be used to to create scripts to simulate the behavior of systems thereby satisfying the requirement of a surrogate engine. These mock service surrogates allow verification by providing an otherwise absent execution trail or returning courtesy messages of the test outcome.

it is used in conjunction with ANT, which while not necessary, is an industry best practice and contains advantages cross-system configuration.

Specification of sufficient accuracy for deriving test cases were not available, and therefore test scripts are based on capture. Test cases are written in the soapui format but since their structural composition is largely that of an XML file, their reusasability and low risk of vendor lock-in is justified. Execution traces are simply full text accounts of test results. 

\section{Evaluation of the framework}
The framework is evaluated mainly on qualitative measures. is done by comparisons to idealized models, interviews with Wärtsilä's experts, and observations of the framework in simulated action.
...
\item test take care of all their housekeeping such as environmental set-up and clear-down
\item it is easier to add a new test to the automated pack than to run that test manually
\end{itemize}

The framework satisfies the first criteria easily. Tests can be grouped and executing packages of tests in Jenkins is done with a simple click. 

The second criteria is also arguable satisfied since integration testing. Is satisfied by inputting information required by the test first and lastly deleting the information. Alternatively, unchanged information may be used, though this is a more risky approach. 
