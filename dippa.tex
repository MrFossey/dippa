...
Deriving test cases from models or using models to represent the testing strategy is known as model-based testing and uses specifications or program code to first derive the model \citep{pezze2008software}. An accurate model has the benefit of reducing specification errors and misinterpretation by providing a common point of reference.

A model can be used to implement automated testing or test case creation, and elicit possible execution paths and plan coverage, and ensure that the system is adequately tested. For istance, a finite-state machine is a formal model which is used...
Some systems, like those in banking and medicine, have very high availability requirements and therefore take advantage of formal models. Models can also be less formal, for instance class diagrams. \citep{pezze2008software}

Deriving test cases from models or using models to represent the testing strategy is known as model-based testing. It uses specifications or program code to derive the model \citep{pezze2008software}. An accurate model has the benefit of reducing specification errors and misinterpreation. A finite state machine may, theoretically, have nn inf

There are benefits in reduced specificaitone errossr and vaoided misinterpratation, theoreticl number of states infinite Guide slection, specs are lacking, is the fsm feasible, wieczorek
...
The more formal the model, the better test cases, still support (pezze). 
can be represented by finites state machine, modeling all transitions involves work, out of scope. for rigorous testing
pezze 245 models extracted from code or specs
