...
The execution engine runing the tests, SoapUI, also collects execution traces. These are transformed into legible form (simple text) and can be examined in Jenkins. Accordinly, what can only be called an initial oversight, visualizing the test results is added as a process to the overall framework process.

SoapUI is an IDE, or integrated development environment, and is also used in constructing test scripts, both manually and through capture and playback. Capture can also be used to to create scripts to simulate the behaviour of systems thereby satisfying the requirement of a surrogate engine.
