...
The bigger and the more numerous information systems are, the more problems are likely to arise. Software failures may result in squandered time and money, and even loss of life. The general reasons for testing automation are improving software quality and cost-efficiency.  Harnessing the power of computers helps expedite the testing process and reduce the amount of manual work.

Integration testing is particularly important at Wärtsilä because the company typically outsources IT development. This pushes unit testing resonsibilities to the supplier, while integrating the solution remains Wärtsilä's concern. A good process framework can serve many development projects to come.
...
Wärtsilä's business relies on over 50 different systems, which are intersected by information exchange processes. The process chains depend on many different systems, for example ERP, PDM, CRM, warehouse control, and financial systems. The architeture is built on the service-orientation principle (SOA) where individual systems lend themselves to be called by other services through a shared channel, the enterprise service bus (ESB).
...
<Wärtsilä's SOA arhitecture>

In a recent ERP update effort it became clear to Wärtsilä that testing is a time-consuming effort and that a framework guiding the testing process and automating a part of it could significantly reduce costs and risks. 

Move general stuff about testing here?

\chapter{Looking for a testing framework}
\label{chapter:integration testing} % TODO

From an IT or software perspective, a framework is sometimes understood as a technology or library intended for a purpose \citep{johnson1988designing}. In the scope of this thesis, the framework concept is extended to the overall process workflow related to (integration) testing. While a technology may be a part of the framework, the framework itself is technology-agnostic and only sets requirements for functionality. \citet{liu2009unified), \citet{huang2008surrogate}, and \citet{laukkanen2006data} present frameworks that span the integration testing process start to end. These are presented in figures \ref{fig:liu}, \ref{fig:huang}, and \ref{fig:laukkanen}. The models show that in actuality, frameworks combine different tools for different steps along the workflow chain.
...
The \citet{liu2009unified} model covers test case generation, case execution, and execution trace collection. The model was used in relatively small-scale experiments, but successfully uncovered bugs early in development. The framework helped find different kind of bugs and didn't appear to have a bias towards uncovering a certain kind of defect \citep{liu2009unified}.
...
Huang et al's model, a precursor to the authors unified testing framework presented above, is more concerned with the testing environment arrangements, but \ref{fig:huang} already shows a clear framework structure spanning from test case generation to verification of functionality. The area around simulating components is the target of focus. Both models attempt to elaborate the test case creation phase. \citep{huang2008surrogate}
...
Where the two first models essentially depict a testing loop and treat continuous testing as implicit, Laukkanen's model instead offers a monitoring step which overlays verification processes. A view to system status and health is more clearly seprated from the testing execution. Naturally, a batch of test cases can be set to run to update the status. Laukkanen's model supports the notion that a full framework should contain tools for test case generation, test execution, and verification/monitoring.
...
\ref{fig:funnel} presents a testing framework wireframe divided to four parts. Testing objectives and the general approach are defined in a testing plan or strategy. Typically, a plan includes ... (reference). The reason the framework definition and strategy are part of the figure is because they not only affect structure, but the way the framework is used.
...
The testing process can vary in its mode of execution or degree of automation. In what is perhaps the most extreme case, test case are run automatically everytime a change is committed to system source code. This approach is called continuous integration (CI). 

Testing outcomes may 

Pace of integration, rate of automation, 

\begin{figure}
\centering
\begin{tikzpicture} [
   start chain=going below,         % General flow is top-to-bottom
    node distance=10mm and 70mm,    % Global setup of box spacing
    ]
% ------------------------------------------------- 
\tikzset{
  base/.style={draw, on chain, on grid, align=center, minimum height=4ex},
  ...
  line/.style={draw, thick, -latex'},
  test/.style={draw, dotted, -latex'}
}

    \node [rect, xshift=-2.0cm, yshift=1.5cm] (stg) {Strategy};
    \node [rect, yshift=0.25cm] (req) {Requirements \& input};
    \node [rect, yshift=-0.75cm] (prc) {Process};
    \node [rect, yshift=-1.4cm] (rlt) {Results};
    
    \node [rect, right=of stg] (fmw) {Framework};
    \node [rect, right=of req] (tcg) {Test generation};
    \node [rect, right=of prc] (run) {Running tests};
    \node [rect, right=of rlt] (mon) {Monitoring/verification};
    
    \node [rect, right=of stg] (art) {Artefacts};
    \node [rect, right=of req] (tcs) {Test cases};
    \node [rect, right=of prc] (exe) {Execution engine};
    \node [rect, right=of rlt] (out) {Testing outcomes};
    
    \node [rect, xshift=-3.5cm] (pit) {Pitfalls};
    
    \draw[semithick] (-5,0) arc (180:0:3 and 0.5);                  % cone 1: top ellipse top arc
    \draw[dashed,color=gray] (-3,-5) arc (180:0:1 and 0.25);        % cone 1: bottom ellipse top arc
    \draw[semithick] (-3,-5) arc (-180:0:1 and 0.25);               % cone 1: bottom ellipse bottom arc
    \draw[semithick] (-5,0) arc (-180:0:3 and 0.5);                 % cone 1: top ellipse bottom arc
    \draw[semithick] (1,0) -- (-1,-5);                              % cone 1: right side
    \draw[semithick] (-5,0) -- (-3,-5);                             % cone 1: left side
    
    \draw[semithick] (2,0) arc (180:0:3 and 0.5);                   % cone 2: top ellipse top arc
    \draw[dashed, color=gray] (4,-5) arc (180:0:1 and 0.25);       % cone 2: bottom ellipse top arc
    \draw[semithick] (4,-5) arc (-180:0:1 and 0.25);               % cone 2: bottom ellipse bottom arc
    \draw[semithick] (2,0) arc (-180:0:3 and 0.5);                  % cone 2: top ellipse bottom arc
    \draw[semithick] (8,0) -- (6,-5);                               % cone 2: right side
    \draw[semithick] (2,0) -- (4,-5);                               % cone 2: left side
    
    \draw[semithick] (2,0) arc (180:0:3 and 0.5);                   % cone 3: top ellipse top arc
    \draw[dashed, color=gray] (4,-5) arc (180:0:1 and 0.25);        % cone 3: bottom ellipse top arc
    \draw[semithick] (4,-5) arc (-180:0:1 and 0.25);                % cone 3: bottom ellipse bottom arc
    \draw[semithick] (2,0) arc (-180:0:3 and 0.5);                  % cone 3: top ellipse bottom arc
    \draw[semithick] (8,0) -- (6,-5);                               % cone 3: right side
    \draw[semithick] (2,0) -- (4,-5);                               % cone 3: left side
...
\end{tikzpicture}
\caption{Framework and strategy funnels} \label{fig:funnel}
\end{figure}
