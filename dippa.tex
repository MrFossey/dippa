...
% todo motivation
% add something about cost-savings because testing and maintenance account for most software costs, reid and myers
...
The first technical expert said that for her, testing often consists of code inspection, a white-box testing method not investigated or included in the framework. Code inspection is done by the developer to find issues in the code directly, without actually executing it. The expert also added the lacking \emph{negative testing}, or testing cases that are expected to fail to see if ill-formed input is able to disrupt systems, as potentially harmful. The remaining parties quietly agreed with the asserted reality that testing is largely ad hoc and driven by each project in its own way.
% not a requirement per se

The testing expert and technical expert jokingly referred to testing being a mandatory step whose purpose is not so much to uncover bugs but to prove systems are ready for the next go-ahead. While the veracity of such a claim is dubious, a framework can serve to ensure proper testing by communicating that negative cases should also be added to testing scope.

\subsection{Requirements validation}

Following the introduction to Wärtsilä testing, the collected framework requirements (see Section \ref{section:requirements}) were shared with the intent to validate them again, and determine the most important ones. The first technical expert put most weight on the test generation requirement (R3). Concern centered on the labour cost of creating tests and generation was crucial in making larger scale testing economically feasible. While the generation requirement may be argued to be not the most important component of a testing framework, it clearly sparked excitement among the participants. The requirement has a lot of potential for gains.

The testing expert was more concerned if the framework actually provided any actionable results. Therefore, the automated verification step (R6) was named the most important. If the framework could not contribute to fixing defects in the integrations and making systems better, it would have little value. The discussion continued on the theme of reporting failures. The testing and first technical expert toyed with the idea of creating automated bug reports in the framework and forwarding them to the bug tracker. This was added as requirement twelve (R12).

% was not added to the list of requirements
% bug tracker not in framework scope
The second technical expert agreed citing both requirements as the most important. Meanwhile, the partner members underlined the importance of performance tests.

Throughout the disuccsion the addition of new requirements was welcomed. Along with automated bug reports, one clear new requirement was version control. Systems and environments go through changes and tests results always pertain to one particular version. This information must be available. Moreover, it is useful to document in which version of the system and environemnts the capture test cases or surrogates were generated. Version control was added as requirement eleven (R11).

\subsection{Framework use and maintenance validation}

Technical expert pointed out that the model explicitly prescribed a database where any filing system or repository would have sufficed.

The issue of framework maintanence was raised by the testing expert. Because the proposed framework required test generation using capture, test resources would have to be updated after any change in the systems' behavior that would render them moot or possibly even misleading. This was also a principal concern in criticism of capture and replay. TODO reference. After discussing the matter with the technical experts the workshop participants concluded that test reusability would be high because test cases would only have to change very infrequently. Hence, capture was accepted as a cost-effective way to create test resources.

alternative: using the existing monitoring tool. contains logs (capture) of messages and their output. resend possible. leaves question of performance testing open.

\subsection{Summary}

Negative testing, while not a requirement of the framework per se, is a testing approach and does not require special attention from framework design. Being able to execute test cases is enough.
...
\item REQ11 Version control in naming test resources (cases, environments, surrogates)
\item REQ12 Integration to bug tracker (email report)
\item Negative testing
\item Pilot framework in project
\item Align framework in Wärtsilä way of working
\item Code inspection as a way of testing? Can it be automated? Most IDEs have syntax control.
\item Test library rather than database
\item Update divergent links, highlight two phases
\item Test coverage who supports who tests, concern over test tool selection
\item skipped, reusability
\item Reusing pre-existing resources like 
\end{enumerate}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
One of the fundamental difficulties in developing an automated integration testing framework is making sense the technology and manual process relationship. The two are haplessly intertwined. Clearly as a framework for \emph{automated} testing the framework is realized by algorithmics and brute computing power, but such a tool cannot be wielded successfully without human intervention and maintenance. It is this blurred relationship of man and machine, the delicate mix of technological configuration and human work process model which tries the framework designer and motivates the researcher. 

Harking back to \citep{pezze2008software} definition of an circuit board with empty slots for components is useful here. The framework consists of roles to be filled by applications or manual processes, the choice being left to each individual testing project. TODO: lähde prosessisuunnittelusta. The framework will fail if it is too narrow and imcompatbile or too broad and poorly supported.

% manual challenges and adaptin the framework
Confirmed in the empirical part the thesis, an essential challenge is introducing the framework into the company's way of working. Testing requires the coopereation of many individual stakeholders. In Wärtsilä's case this contribution was seen as extra work for solution managers and soliciting their support was difficult. Here, a clearly defined framework which states what resources or contributions are needed can bring credibility and trust to the project and provide a channel for coopration. Testing is step that repeatedly crops up in software development projects and having a documented way to do so is advantageous. 

% framework bonuses
The kicker to having a documented process and automted testing framework is the reusabilty of test resources like test cases, also a prerequisite of automation, which may act as an incentive to contribute to the framework. This remains to be seen. Admittedly, casess will have to be maintanted and updated, but the frequency was deemed to be low by Wärtsilä's experts.

% generalizable
The thesis asserts that the devised framework presented in \ref{chapter:framework} is generalizable and can benefit any party doing testing based on capture and replay. (Choosing the exact tools is a separate task and dependent on contextual factors of each individual case, however.) Testing as a process, is cemented in three steps, preparing necessary input and conditions, executing tests, and verifying results, perhaps with the addition of a clean-up step  logging test results or creating test reports.

% component testing replacing integration testing
Software is abstracted and managed by splitting it to blocks. These blocks form a specific functional area that is a thematical whole and which can be assigned to responsible and which generally make the software easier to manage. In fact, integration might be thought to be not between systems but between software components, which respects interfaces more broadly. One may ask if integration testing scope will become too bloated in this vein of thought any trasaction with two points constituting an "integration". 
...
Capture and playback was the target of criticism but many commercial and non-commercial tools are based on it and while theoretically it may not be the best option, it arguably satisfies most companies' testing requirements and needs, at least temporarily. It remains to be seen if public opinon shifts after more is known about the lifecycle of such a tool and the maintenance requirments of such a strategy. The alternative, model-driven testing, requires an initial investment that is greater, but it might pay off. Perhaps system modelling will become so easy, and the philosophy of separating content with structure so prevalated as to enable test case and test data creation based specifications.

In practice, integration testing might be skipped entirely in favor of end-to-end system tests which are thought to cover testing integrations. Once again testing is squeezed. Therefore, another area begging attention is whether integrartion testing should be conducted through end-to-end testing and what are the potential upsides and downsides. More accurately, this is to ask if integration testing should simply test the integration between two systems or if a whole chain of trans-system communication should be packaged into a test case. An end-to-end test case will give more coverage per case, and perhaps a more realistic test scenario, but also include a larger testing scope. If something goes awry can the test engineer or developer easily see what's wrong? Duvall advocates including only one assertion per case, which is counterproductive relative to the scale of an end-to-end case. If only a portion of system communication is relevenat, the rest goes to waste.

Future research might also labour over collecting, combining, and comparing strategies for test case creation. As such, experiments and proof of concepts have been given of generating test data (todo reference), creating test case tempaltes automatically (todo wsdl reference), and capture strategies (references). 

Rehmann has done commendabable work stepping back to examine integration testing through fresh eyes. Different alternatives for testing \emph{component integrations} are laid out and compared. A similar unbiased and novel approach should be applied to reappraising the tired old big-bang.

More focused areas like fault-based testing. The framework might supplemented by a set guidelines to include fault-based test cases, i.e. instructions for creating tests cases based on defects detected in the system.
...
%mixing manual and technical aspects
One of the fundamental difficulties in developing an integration testing framework is making sense the intertwined relationship of technology and manual work involved in and around it. Clearly as a framework for \emph{automated} testing the framework is realized by algorithmic and computational execution, but it cannot be used successfully without human intervention and maintenance. It is this blurred relationship of man and machine, the delicate mix of technological configuration and human work process model which tries the framework developer. \citep{pezze2008software} definition of an cirtcuit board with empty slots for components.

% manual challenges and adaptin the framework
In the empirical part an essential challenge was introducing the framework into the company's way of working. Testing requires the coopereation of many individual stakeholders. In Wärtsilä's case this contribution was seen as extra work for application managers and soliciting their support was notoriopusly difficult. Here, a clearly defined framework which states what resources or contributions are needed can bring credibility and trust to the project and provide a channel for coopration. Testing is step that repeatedly crops up in software development projects and having a documented way to do so is advantageous.

% generalizable
The devised framework is generalizable and can benefit any party doing testing based on capture and replay. Choosing the exact tools is a separate task and dependent on contextual factors of each individual case, however. Testing as a process, is cemented in three steps, preparing necessary input and conditions, executing tests, and verifying results, perhaps with the addition of a cleaning step taking logging test cases or creating test reports.


It is not possible to go through every code branch or to test every possible input sequence. As \citet{myers1976software} put it, testing is a problem in economics. Test cases must provide a maximum yield on investment, where yield is measured as the probability that the case uncovers a new defect. \citep{myers1976software} 

One of the essential findings of this thesis is that Continuous Integration actually bridges the gaps in integration testing theory when examining the entire software lifecycle. Authors like Myers and XX and textbooks () still continue to perpetuate the image that integration testing is mere a software component assembly task and a simply a staging post in software development. This is unfair because integrations should be tested regularly when there is even the slightest change in systems composition or source code. CI connects integration testing with functional, non-functional, and regression testing while adhering the tester's adage "test early and often". 
% also automation



Good testing is based on good plans and ensured by good governance. Plans include specifications of systems and models of their behavior. Technology has even taken steps to facilitate making these plans actionable, for instance separating structure and content. Where the structure can be seen as a generalizable asset repllicated in tests and content being test data. A good exmaple of this is web services where a single specification file can be used to automatically generate an executable case. However, test data and verification (adding assertions and the like) must be done by a test engineer, still. It remains a task for human discretion to decide what is to be tested and what the testing objectives are.
...
CI a highly practical philosphy bridging the gap between integration testing (strategy) and regression testing.
