...
The first technical expert said that for her, testing often consists of code inspection, a white-box testing method not investigated or included in the framework. Code inspection is done by the developer to find issues in the code directly, without actually executing it. The expert also added the lacking \emph{negative testing}, or testing cases that are expected to fail to see if ill-formed input is able to disrupt system operation. The remaining parties quietly agreed with the asserted reality that testing is largely ad hoc and driven by each project in its own way.

Following this, the collected requirements (see Section \ref{section:requirements}) were shared with the intent to validate the requriements again and determine the most important ones. The first technical expert put most weight on the test generation requirement (R3). Concern centered on the labour cost of creating tests and generation was crucial in making larger scale testing economically feasible. While the generation requirment may be argued to be not the most important component of a testing framework, it clearly sparked excitement among the participants. The requirement had a lot of potential for gains. 
