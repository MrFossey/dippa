...
In the surrogate system, the environment is made of the application container which may or may not contain parts or the whole of the system under test and in which surrogates, or mock services or stubs, are deployed. Again, the many interfaces in the framework are highlighted.
...
There are a total of First off, messages captured in the production environment are used for the generation of new test cases. This means that in the framework solution there must be some way to \emph{record} behaviour in production. In the following \emph{generation} step, the records are turned into an executable test case, which are then \emph{run} by a testing engine against the system under test. The generation phase is likely the most labour-consuming and must be planned and supported by clear documentation and accessibility to necessary tools.
...
\node [file] (rec) {Message record};
\node [proc] (gen) {Integration test case generation};
\node [file] (cse) {Test case};
\node [syst] (exe) {Test case execution engine};
\node [sut] (sut) {System under test};
\node [syst] (eng) {Surrogate engine};
\node [proc, right=of f1] (ver) {Verification};
\node [file, right=of e0] (ext) {Exe-cution trace};
\node [proc, right=of s1] (col) {Trace collecting};
\node [proc, right=of e1] (sgt) {Surrogate generation};

\path [line] (cap) -- (rec);
\path [line] (rec) -- (gen);
\path [line] (gen) -- (cse);
\path [line] (cse) -- (ver);
\path [line] (ver) -- (gen); % Todo: what is this?
\path [line] (cse) -- (gen);
\path [line] (gen) -- (sut);
\path [line] (sut) -- (eng); 
\path [line] (sgt) -- (eng);
