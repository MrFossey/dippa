...
The framework satisfies the first criteria easily. Executing packages of tests packages in Jenkins is done with a simple click. Likewise choosing which tests to execute is not uncomplicated. Integration test cases can be organised inside Jenkins projects so that only the needed suite(s) of cases is run. This does include some mainantenance work, however. Knowing which projects should be appended with the new case. This issue can be subverted by guidelines already at the test creation process so that the test engineer adds the case in the correct test suite. The framework adminsitrator shouls see to that each suite is correctly linked to projects.
