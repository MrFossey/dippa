...
\begin{tikzpicture}[
    ...
    node distance=16mm and 6mm, % Global setup of box spacing
    ]
% ------------------------------------------------- 
\tikzset{
  ...
  blank/.style={base, rectangle, minimum height=8em, text width=10em},
   line/.style={draw, thick, -latex'}
}
% -------------------------------------------------
% Placing the nodes

\node [blank] (tpt) {Theory path};
\node [proc] (thr) {Theory};
\node [proc] (anl) {Analysis};
\node [proc] (syn) {Synthesis};
\node [proc] (con) {Conclusions};

\node [blank, below=of tpt] (ept) {Empiric path};
