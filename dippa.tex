...
Performance testing in the framework made possible by SoapUI’s sister tool \textbf{LoadUI} \footnotte{\url{http://www.loadui.org/}}. LoadUI was chosen because its configuration, including execution of tests and integration to Jenkins, is analogous to the SoapUI configuration. Performance tests based on SoapUI test cases can also be configured with the tool’s graphical UI eliminating the need to create new performance test cases. Assertions, such as threshold limits for response time, can be added to the cases to establish pass/fail criteria. As with SoapUI, the tool provides a program which can be run from the command line with the test run details and configuration settings as parameters.
