...
\tikzset{
  ...
  file/.style={base, cylinder, aspect=2, text width=5em},
  engine/.style={proc, rounded corners},
...
\node [system] (p0) {Production};
% Use join to connect a node to the previous one 
\node [file]      {Model/diagram};
\node [proc, join] (p1) {Test case generation};
\node [proc, join]      {Integration test case};
\node [proc, join]      {Verification};
\node [test, join] (t1) {Test case execution engine};
% No join for exits from test nodes - connections have more complex
% requirements
% We continue until all the blocks are positioned
\node [proc] (p2) {SUT};
\node [proc, join] (p3) {Surrogate engine};
\node [test, join] (t2) {Execution trace};
\node [test] (t3) {Trace transformation};
\node [test] (t4) {Trace collecting};
% -------------------------------------------------
