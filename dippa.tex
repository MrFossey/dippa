...
In \citeauthor{laukkanen2006data}'s \citeyearpar{laukkanen2006data} keyword-driven testing model (figure \ref{fig:laukkanen}) testing is divided into modules for \emph{test design}, \emph{test execution}, and \emph{test monitoring}. Where the two first models essentially depict a loop and treat continuous testing as implicit, the key-word driven model instead offers a monitoring step which overlays verification processes where a view to system status and health is more clearly separated from the testing execution. Naturally, a batch of test cases can be set to run to update the status. \citeauthor{laukkanen2006data}'s \citeyearpar{laukkanen2006data} model supports the notion that a full framework should contain tools for test case generation, test execution, and verification or monitoring.
...
can be represented by finites state machine, modeling all transitions involves work, out of scope. for rigourous testing
...
benefits in reduced specificaition errors and avoided misinterpretation 
often simplications although theoretical number of states infinite
state machine guides selection
often specs lakcing, missing cases implicitly error cases
...
Virtually any scripting language is capable of executing scheduled commands and the technical automation challenge is not as interesting as test case generation or oracle construction. Therefore, more attention is paid to latter.
