...
\\
Answering the third research question involves proofing the selected frameworks. This is a smoke test of the suitability of the chosen framework, but the actual implementation of the test framework and adopting is not part of the thesis. \\
\end{adjustwidth}
...
From an IT perspective, a testing framework is understood as a technology tool or library intended for testing (REFERENCE). In the scope of this thesis, however, the framework concept is extended to the overall process workflow related to (integration) testing. While a technology may be a part of the framework, the framework itself is technology-agnostic and only sets requirements for functionality. Liu and Huang present frameworks that span the integration testing process start to end. These are presented in figures X and Y. The models show that in actuality, frameworks combine different tools for different steps along the workflow chain.
...
Liu et al model covers test case generation, case execution, and execution trace collection. The model was used in relatively small-scale experiments to successfully uncover bugs earlier\ldots
...
Huang et al's model, a precursor to the authors later model presented above, is more concerned with the testing environment, but figure Y already shows a clear framework structure. 

This chapter explores the theoretical base for a framewor. First it examines integration and testing strategies in general in respect to frameworks. Then the requirements for the framework are defined. Next, the framework's testing process and outcomes . The final subchapter looks at common integration testing pitfalls and how they can be avoided. 
...
Traditionally, testing is grouped to unit, integration, and system levels. Unit testing is assuring that individual software components and functions perform to specifications. When components are paired or grouped, the resulting connections must also be tested. Finally, system testing refers to testing the finished software suite with a suitable array of use cases.
