...
\node [proc] (tcs) {Configured test case};

\node [proc, right=of desc] (gen) {Generator};
\node [proc] (ten) {Test engine};

\node [proc, right=of gen] (app) {Application container};
\node [proc] (sgt) {Surrogate};
\node [proc] (rea) {Real component};
...
\end{tikzpicture}
\caption{Aurrogate system architecture \citep{huang2008surrogate}} \label{fig:surrogate}
\end{figure}
