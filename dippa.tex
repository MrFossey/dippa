...
ANT & Apache Ant is a Java library and command-line tool whose mission is to drive processes, such as compiling, assembling, testing, described in build files as targets and extension points dependent upon each other. \citep{ant2013} \\[0.3cm]
...
IDE & \textbf{I}ntegrated \textbf{D}evelopment \textbf{E}nvironment.  \\[0.3cm] 
...
% just a technical requirement todo
...
\begin{center}
    ...
    \end{tabular} 
\end{center}
\caption{Integration testing errors, adapted from \citep{leung1990study}} \label{fig:errors}
\end{figure}
...
The Evaluation chapter compares the proof of concept construct to external requirements and guidelines and gives an estimate how well 
...
All hallmarks of constructive research were met successfully. The thesis brought a real problem under theretical scrutiny and generalized it into a model which was subjected to tests.
...
The distilled wisdom to come is not argued in lengthy detail and anyone interested in additional information or analysis is invited to read the thesis which is available at: http://dippaeditor.com/preview/eVW29cQ7gwyM/
...
The test process is divided in to a preparation step where test resources are created and an execution step in which test cases are run and rerun as the step is repeated. The whole process is pictured in Figure 1.
...
\begin{enumerate}
\item Plan coverage and relevant measures beforehand.
...
\item Draw call graph to visualize integrations.
...
\textbf{Tests that are explicitly deemed important.} E.g. some area that critical, has been recently changed, is prone to bugs or is known to have caused failures.

\textbf{A set of breadth tests that roughly cover all features in test scope.} This will give minimum satisfaction of test objectives. 

\textbf{Test for most important functions.} Function importance may be attributed to e.g. message traffic, financial impact, etc.

\textbf{Tests that are easy to automate.} Good for quick wins and return for investment. Also quickly ramps up the framework construction.

\textbf{Test that will give quickest payback.} Implement test cases in projects which would otherwise require a lot of manual testing.

\textbf{Test that are run most often.} If tests need to be run often, the tested area is likely subject to many changes thereby increasing risk.


Example 1 – Configuring SoapUI with web service
See figure 2.
1.	If you already haven’t, install the free version (not the pro version) of SoapUI from http://www.soapui.org/.
2.	For a SoapUI sample integration create a new project in SoapUI. Right click the project and choose “add WSDL” and add the web service description (e.g. the one attached to this document).  This creates a web service binding with a sample request. 
 
3.	The request is missing test data which must be added. For the attached example service for instance, insert the following in the fields with ‘?’ in this order: ‘3333’, ‘32’, ‘GEN’, ‘False’, ‘True’.
4.	You must also add username ext_sco and password wm7qa in the ‘Aut’ authentication tab.  The test client must have the rights to access SUT services.
5.	Add the correct endpoint URL in the request edit window, e.g. for the example service: http://wesbqa.wartsila.com:5557/ws/sco.common.ws:SCOServiceProvider 
6.	 Now the request can be run by clicking the green arrow. You should get a return message with engine information.  
Example 2 – Creating and running a test case
See figures 3, 4, and 5.
1.	Right click the project you created and choose “launch SOAP monitor”. Use the default port and check the checkbox “use as global proxy”. 
2.	Then open the sample request with test data and run it. This should create a log entry in the monitor window. 
3.	In the monitor window click the button “add selected requests to test case”. Create new suite and test case in the wizard. Don’t leave any spaces in their names however!
4.	 Now, if you open the test case and test step you can find “monitor request 1”. Add an assertion or two (click ‘Assertions’ to open the assertion view), like SOAP message or SLA level. Run the case again to see that it passes.
5.	Try running the case by right clicking the project and choosing ‘launch testrunner’. Run the new suite and case. The subsequently opened dialog provides the command with parameters to run the case from command line (it is at the top, scroll there).
Example 3 – Configuring Jenkins
See figures 6 and 7.
1.	Installing Jenkins is easy. Just download the war-package and deploy it on the test machine. Follow the instructions at http://jenkins-ci.org/. Afterwards open Jenkins at localhost:8080.
2.	 In Jenkins, choose new job, give it a name, choose freestyle project and OK. 
3.	Next, configure the Jenkins project to run windows bat file at every build. Paste the command given by SoapUI testrunner and before it add a line were you change directory to the folder where SoapUI’s testrunner.bat is located. For the setup to work SoapUI must be installed on the same machine as Jenkins.
4.	Building the project is now possible. If you keep Jenkins running, you can schedule builds, too, in the project configuration window.
