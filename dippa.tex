...
The first technical expert said that for her, testing often consists of code inspection, a white-box testing method not investigated or included in the framework. Code inspection is done by the developer to find issues in the code directly, without actually executing it. The expert also added the lacking \emph{negative testing}, or testing cases that are expected to fail to see if ill-formed input is able to disrupt systems. The remaining parties quietly agreed with the asserted reality that testing is largely ad hoc and driven by each project in its own way.
