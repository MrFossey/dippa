...
Verification is the step that applies value to test execution. It transforms data from test runs to information with meanining. This is done through \emph{comparing} the execution outputs with expected outputs. Colloquially, the step is often associated with test oracles, though \citep{fewster1999testing} argue for the term comparison since automated algorithms cannot tell about success of failure, but only apply rules to the test results.

Simple comparison looks for an identical meatch between ouutput and expected output. The apparoch is popular but inflexible. Complex comparison takes into account known difference in test results. 
