...
The oft-cited reasons for testing automation are improving software quality and cost-efficiency: harnessing the power of computers helps expedite the testing process as well as reduce the amount of manual work. \citep{laukkanen2006data} give a more complete list of automation benefits: %TODO

Rerunning tests on a new version of the system
Reusing tests in a new project
Running tests more often
Perform tests which would be difficult or impossible to do manually (e.g. performance tests}
Freeing test engineers to do more rewarding work
Increased confidence due to increased and systematic testing
...
% add reference
Since the systems undergo continuous changes, like improvements, upgrades, fixes, pilots, etc., the need for regression tests is common. Ideally, a prepared test set could be executed by issuing a simple command. In addition to the benefits listed above, testing, if executed continously, would also provide an up-to-date view on the status of of systems. This improves quality and transparency of information and can thereby argued to aid decision-making.

Current testing at Wärtsilä is ad-hoc. There is no universal testing framework. A set of tools, which include for instance a bug tracker tool and wep application automation, have been adopted and are used on a mix-and-match basis. Testing strategy and policy as well as testing processes are described and defined, but the practical testing approach is set by project managers or project test managers.

Requirements and specifications are collected among solution key users and managers. Unit tests are performed by the vendor, which is followed by integration tests in Wärtsilä's test environment executed by Wärtsilä's technical experts possibly aided by the vendor. Next, system tests are done by the information management solution manager, who then passes the solution forward to business key useres for verification testing. 

Test cases are constructed manually and are not maintained. After a development project is closed and moves to live maintanence mode, new cases are created as needed when new features or enhancements are released. In some cases, a battery of regression tests is maintained. Testing results and found incidents are logged to a bug tracking system, or maintained by shared files or email conversations. 

Remarkably, the reusability of testing artefacts is minimal. Steps have been taken to standardize the testing process, to identify the most importnat points of improvement, and raise the maturity level of testing and esatblish the preconditions for an automated and controlled organization-wide process.

%maintanence work, involving people and gettim them to comply.

% resources, frequency, results, guidelines, test cases, yearly increase, 
...
\section{Automation in testing} % execution in framework
...
Researchers like \citet{zallar2001you} and \citet{kit1999integrated} criticize in particular the capture/replay paradigm where a test case is recorded so it can be automatically reproduced and run afterwards. Maintaining the test scripts is cumbersome and as such will often be left undone rendering the test cases unusable and potentially misleading \citep{zallar2001you}. \citet{kit1999integrated} suggests to just leave out capture and simply use automation to replay.
...
\section{Evaluating testing} % creating test cases
...
\citet{linnenkugel1990test} present a similar coverage hierarchy of node, relation, call, sequence, and loop coverage measures. % edges and vertices
