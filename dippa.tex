...
\citet{linnenkugel1990test} present a flow-oriented coverage hierarchy of node, relation, call, sequence, and loop coverage measures. For integration testing, individual components (modules) are nodes and integrations are relations(call statements). Complex path sequences can be interpreted as system tests. 
\begin{itemize}
...
\item \emph{all-multiple-relations} requires that every call between modules is executed at least once, i.e. that every edge of the p-graph is covered (all-edges).

all-simple-call-sequences requires that every sequence of (descending) calls without repetition of calls is executed at least once, i.e. that every simple path of the p-graph is covered (all-simple-paths).
