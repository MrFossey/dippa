...
problems than benefit. Testing reduces the number of incidents that would otherwise be discovered in the live environment and fixing which would be very expensive. In a similar fashion, testing can point to ways the systems and their use can be improved. A system that is easy to use does not incur service and information requests. (RISK)
...
Delaying testing to the Big Bang is not ideal for a large and complex integration undertaking. A better option is to test little by little and employ one of the other schemes. This type of testing requires using drivers and stubs to 
