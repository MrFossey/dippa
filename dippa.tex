...
This master's thesis study is divided to x parts. First, necessary background information on software integration testing, Wärtsilä's systems architecture, and information systems is presented in Chapter \ref{chapter:background}. Next, integration testing frameworks are explored by looking at existing models and a synthesis on the strategic, is given. Along with a more generic look at software testing theory, Chapter \ref{chapter:framework} breaks down the framework into process components which are then put together in a new framework model tailored for Wärtsilä in Chapter \ref{chapter:frameworkproposal}.

definitions of a testing framework, their workflows, and purpose are provided. Second, the approaches for testing automation are examined. Thirdly, the overall of the testing setup is evaluated from the perspective of efficient defect management. 
