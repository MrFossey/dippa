...
Where the two first models essentially depict a testing loop and treat continuous testing as implicit, Laukkanen's model instead offers a monitoring step which overlays verification processes. A view to system status and health is more clearly seprated from the testing execution. Naturally, a batch of test cases can be set to run to update the status. Laukkanen's model supports the notion that a full framwork should contain tools for test case generation, test execution, and verification/monitoring.
