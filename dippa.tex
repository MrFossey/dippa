...
Test cases are constructed manually and are not maintained. After a development project is closed and moves to live maintanence mode, new cases are created as needed when new features or enhancements are released. 

In some cases, a battery of regression tests is maintained.

as is the collection of requirements, and the testing process itself. Testing is done when needed, usually after some change or update has been implemented in one or more of Wärtsilä's or Wärsilä's partners' systems. Test cases and results are collected in documents, but no material is explicitly made reusable. Results are used for decision-making\ldots 
