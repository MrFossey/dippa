...
At the beginning of the meeting, participants were presented with a mission statement and a short introduction to the topic explaining the reasons that led to commissioning an automated testing framework study. The subject of current testing was brought under discussion. 

The first technical expert said that for her, testing often consists of code inspection, a white-box testing method not investigated or included in the framework. Code inspection is done by the developer to find issues in the code directly, without actually executing it. The expert also added the lacking \emph{negative testing}, or testing cases that are expected to fail to see if ill-formed input is able to disrupt systems, as a challenge. The remaining parties quietly agreed with the asserted reality that testing is largely ad hoc and driven by each project in its own way.
% not a requirement per se

Following this, the collected requirements (see Section \ref{section:requirements}) were shared with the intent to validate the requriements again and determine the most important ones. The first technical expert put most weight on the test generation requirement (R3). Concern centered on the labour cost of creating tests and generation was crucial in making larger scale testing economically feasible. While the generation requirement may be argued to be not the most important component of a testing framework, it clearly sparked excitement among the participants. The requirement had a lot of potential for gains.

The testing expert was more concerned if the framework actually provided any actionable results. Therefore, the automated verification step (R6) was named the most important. The discussion continued on the theme of reporting failures. An idea was of creating automated bug reports through integration with the bug tracker to was presented.
...
The addition of new requirements was also welcomed. One clear new requirement was version control. Running different versions of e.g. surrogates and mock services against possibly different versions of systems was requested.
...
The workshop discussions were also recorded so they could be comprehensively analysed afterwards. 
