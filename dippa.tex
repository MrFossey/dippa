...
Early on in tool selection it became clear that one tool could not handle all processes but conversly not all processes required their own tool. Testing software could possible cater to different, even disparate aspects of the framework. The evaluation of testing tools is omitted from this study. The reader is instead referred to excellent resources such as Opensourcetesting.org and Thoughtworks CI tool comparison matrix (http://confluence.public.thoughtworks.org/display/CC/CI+Feature+Matrix).
