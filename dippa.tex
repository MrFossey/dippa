...
\newcommand{\SUBTITLE}{Case Wartsila} % TODO umlaut
...
problems than benefit. Testing reduces the number of incidents that would otherwise be discovered in the live environment and fixing which would be very expensive. In a similar fashion, testing can point to ways the systems can be improved. For example, a system that is fluid and easy to use does not incur as many service and maintenance or information requests. At the very least testing helps decision makers understand the risk involved. 
% TODO add references above

% Clause about functional and non-functional testing, performance and availability testing
Along with the functionality of the system, tests should also be applied on non-functional aspects such as performance and usability, because these also determine the validity of the system under test.

% What to test, cit huang et al, liu et al
Naturally, software has to be tested to after development, but there are other reasons, too. Any change in code or specifications, the environement, or testing policy or plans can act as a trigger for testing. Systematically repeating a set of tests to ascertain changes (like fixes) have not introduced nre errors to previously functional code is called regression testing. The execution of regression stests is typically automated. A nearby concept, \emph{retesting} refers to examining if a fix has actually resolved an incident (Jenkins 2008).

Similar to regression testing is continous integration testing (CIT), which advocates early integration testing.
...
specifications. When compenents are paired or grouped, the resulting integrations must also 
...
Oriented Architecture esepcially blurs the line between development and integration (Huang et al. 2008; Weiczorek et al. 2010). Myers (1976) introduces six integration strategies: bottom-up, top-down,
...
or the the most important system, whereas in big bang testing testing is delayed until the 
system is wholly integrated, wehere sandwich testing lies inbetween big bang and top-
...
Delaying testing to the Big Bang is not ideal for a large and complex integration undertaking because examining the integration as a whole and pinpointing discovered defects is difficult. A better option maight to test little by little and employ one of the other schemes. However, Integration-by-integration testing requires drivers and stubs as stand-ins to make up for the absence of actual systems needed for tests (TODO).

\section{Automated testing}

Automating testing has obvious benefits, but is not without fault. Implementing automated tests takes a significant investment of time and effort, cannot match the intelligence and intuition of an experienced tester, while some systems undergo so many changes that automation makes little sense. Automation also does not counter inherently flawed software development processes. (Jenkins 2008; Zhang & Zheng)

As changes are introduced into the systems and code, related test cases must examined, their 
reusability ascertained, out-of-date cases should be discarded, and new cases created where 
necessary (Zhang & Zheng 2008).

Test case creation is perhaps the most time-consuming task in automated testing. Cases have to be produced based on the test plan and execution strategy, and updated when necessary. Researchers have presented solutions for automatic test case generation. Tsai et al. (2001) presents an XML-based framework for automtaed test case generation from WSDL service descriptions. Bai et al. and M Di Penta similarly offer WSDL service description based generation strategies. For regression testing, where the system suite is already in place, cases and related test data can be produced by recording actual use cases from production.
...
Bhuyn et al (2012) present selection strategies for regression testing. Testing can be selective where only a part of the whole system is examined in order save time and costs. It is essential to model and understand the system hierarchy and which part of it should be tested. Code-based tests look for modified and related code that is tested, typically in unit tests. Testing can also be risk-based, recalling selective integration strategies. Risk areas are ones most affected by changes and which require the best covereage. In state-based regression testing coverage criteria are determined and test cases  from control and data flow in an UML state diagram and it is shown how to generate test cases satisfying these criteria from UML state diagrams., Lastly, models, such as UML diagrams, can be used implemented regression testing for instance when access to code is not available.

The test set quality itself can be evalauted quantitively and 'tested'. In Delamaro & Maldonado's (2001) mutation testing the test set quality is evaluated by creating variations of the tested programs and running the cases against the mutated programs. If it succeed against the mutated program, the case is considered live. Ideally, cases will fail when run against modified, erronous code.

Regresssion Testing Challenges 
- Lack of observability of Service code and structure
...
Models can serve as basis for testing

UML sequences outline tests and enable automated testing (hura dimmich)

HUANG ET AL and LIU ET AL offer a unified testing framework for testing integration. The 
frameworks contain a generator for test cases, an execution engine, a surrogate engine 
simulating unavaialbe systems. 

This involves creating a set of test cases to be executed on the system under test (SUT). 
<FIGURE> Testing is executed on a continous basis (CIT) to systematically uncover defects. To 
enable integration testing, the model suggests using surrogate systems to stand in for 
incomplete ones.

Model driven integration, Wieczorek & found savings of 50 % .



est case creation through finite state machine, (markov chains) implementation

Thin threads, groupin into a tree (tsai et al)

Test data difficult to select (linnenkugel mullburg)

measures of test effectivess and testability
(how many test cases were executed out of necessary and how many test cases are necessary for 

satifying a criterion)



% Chapter on and introduction to modelling

Modeling is essential to understand and communicate system and component dependencies in 

integration. what connections are there to test? Hewett et al persuades one to lay out and 

quantify components and their connections in a graph and use this information to design the 

most efficient order for testing, with a minimal need for creating test stubs. Not 

dissimilarly, Abdullah et al endorse the concept of a firewall, which draws a line between 

components and systems that are to be tested, and those that need not, based on where changes 

in code or specifications have occurred. Modelling can therefore point to systems that should 

be checked AND those that should be left out of testing scope. This both strenghtens 

understainding of the innerworkings of the tested system set-up and reduces testing costs. 

This kind of testing goes through the system(s) piece by piece while it disregards testing 

entire process chains.

    % add lueng and white for firewall

In particular researchers advocate the creation of a call graph (lueng white, Hulburt, 

linnenkugel), which explicitly maps intearactions and relationships between integrated system 

components. The call graph can be used as a tool to communicate integration structures and 

schemes, and to organize testing. A systematic sweep of components and component connections 

is used to uncover any defects in the integration phase. 

Testing hierarchy and coverage among nodes and links (linnenkugel and mullerburg).

The graph model can also help minimize the resources needed for testing, for example the 

number of stubs (Hewett & Kijsayothin).

-best practice way of visualizing integrations
	existing tool?
	uml
	visio
	bpmn

"Given one or more modules which
have been changed, the firewall encloses the set of
modules that must be retested. The firewull is an
imaginary boundary that limits the amount of retesting
for modified software which contains errors introduced
during the revision process 8,10."

% Chapter on integration errors
...
% List of integration failure points (moll and ammerlaan), add hulburt and typical errors
1. Testing not given importance. Testing considered done when systems connected.
2. No overall integration responsibility. Workflow should identify responsibility and accountability.
3. Physically separated locations.
4. Poor documentation and document management. Essential in enabling good testing.
5. Lack of knowledge. Expert availability.
6. System integration testing as the project's 'crushable zone'
7. Absense of a system integration strategy.
8. Failing configuration management
9. Failing incident management, add this to test workflow in thesis? Use old?
10. Unclear lines of reporting and escalation


Model-based test case generation can help reduce the costs involved and enable systematic 

selection of cases (Benz).

Reid evaluates that about 50% of system development time and more than 50% of costs are spent 
