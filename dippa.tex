...
operation or functional integrity, systems accumulate great risk and may ultimately produce more 
problems than benefit. Testing reduces the number of incidents that would otherwise be discovered in the live environment and fixing which would be very expensive. In a similar fashion, testing can point to ways the systems and their use can be improved. A system that is easy to use does not incur service and information requests. (understanding RISK)

% What to test
Naturally, software has to be tested to after development , but there are other triggers, too. Any change in code or specifications, the environement, testing policy or plans can act a . Continously repeating tests, or regression testing, is i. The repeated and unchanged tests' execution is often automated.
...
% List of integration failure points
