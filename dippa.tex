...
First off, production records are used for the genration of new test cases. This means that there must be some way to capture and record behaviour in the production environment, and save the information in a database. In the following generation step, the records are turned into an executable test case, which are then run by a testing engine. Models, diagrams, and specifications can also be used for automatically producing test cases. Models and specifications should be used to make sure the test cases test and measure the correct things. The different types of services must be turned into cases in a way that the engine can run them. 
