...
Before test cases are produced a strategy for their selection should be selected. Ideally the case set provides minimum required coverage based on the strategy criteria. Benz (2007) lists different schemes for testing strategy and case coverage:
- Manual selection
- Task coverage (each task performed at least once)
- Simple Temporal Relationship Coverage
- Complete Temporal Relationship Coverage
- n-Task combination coverage
- Interruption coverage (interruption of task by ther task)
- random selection
- n-m-Concurrency-Coverage
- path coverage
- transition coverage
- shortest path

Bhuyn et al (2012) present selection strategies for regression testing. Testing can be selective where only a part of the whole system is examined in order save time and costs. It is essential to model and understand the system hierarchy and which part of it should be tested. Code-based tests looks for modified and related code that is tested, typically in unit tests. Testing can also be risk based, recalling the selective integration strategies. Risk areas are ones subjected to changes require more covereage. State, model, regression.
...
- Test only a part of the system (changed part)
- takes less time
- "control flow graph" to identify suitable test scope


Code based regression testing
- modified code and code associated to that are subject to testing
- better for unit test

Risk based regression testing
- A risk area is identified which is considered as the most coverage area that get affected by the changes made in the system.

State based regression testing
- A set of coverage criteria is proposed based on control and data flow in UML state diagram and it is shown how to generate test cases satisfying these criteria from UML state diagrams.
- Control flow diagram

Model based regression testing
- code unavailable
- UML class and sequence diagrams are used to implement Model Based Regression Testing
...
% coverage
There are many strategies for selecting test cases. Ideally the case set is provides minimum 
reqiured coverage based on selected strategy criteria. Benz lists
