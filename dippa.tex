...
Harking back to \citep{pezze2008software} definition of an cirtcuit board with empty slots for components is useful. here. The framework consists of roles to be filled by applications or manual processes. The circuit board prescribes the roles, but each component need only to fulfil the empty slot but can do this in their own way. If the framework is not usable in some testing context, it has failed. If the framework is abandandoed for being too aboslute, it has failed.
