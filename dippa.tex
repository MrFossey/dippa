...
First off, production records are used for the genration of new test cases. This means that there must be some way to capture and record behaviour in the production environment and save the information in a database. In the generation step, the records are turned into an executable test case. The cases are then run by a testing engine. Different types of services must be turned into cases in a way that the engine can run them. 
